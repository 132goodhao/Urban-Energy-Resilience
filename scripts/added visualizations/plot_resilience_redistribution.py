"""Plot resilience redistribution patterns."""
...
def plot_fig6():
    ...
    for prov in common:
        ...
        size = min(max(abs(ne) / 300, 20), 250)

        ax.scatter(ne, rc, s=size, c=color, alpha=0.65,
    ...
    legend_elements = [
        mpatches.Patch(color='#E74C3C', alpha=0.65, label='Net energy exporter'),
        mpatches.Patch(color='#3498DB', alpha=0.65, label='Net energy importer'),
    ]
    ...
    fig.tight_layout()
    save_figure(fig, 'fig6_resilience_redistribution', OUTPUT_DIR)
    plt.close(fig)
    print('Fig. 6 done.')
